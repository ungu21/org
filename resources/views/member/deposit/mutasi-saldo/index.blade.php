...
<section class="content-header hidden-xs">
	<h1>Deposit <small>Mutasi Saldo</small></h1>
   <ol class="breadcrumb">
               ...
               <table class="table table-hover">
                  ...
                  <tr>
                     <td>
                       ...
                       <div style="color:grey;font-style: italic;">Sisa Saldo : {{number_format($data->saldo, 2, '.', '')}} {{$setting->unit_point_name}}</div>
                     </td>
                     <td align="right" style="width:35%;">
                        <div><i class="fa fa-clock-o"></i> <small>{{date("H:i:s", strtotime($data->created_at))}}</small></div>
                        @if($data->type == 'debit')
                        <div style="font-weight: bold;" class="text-danger"> -{{number_format($data->nominal, 2, '.', '')}} {{$setting->unit_point_name}}</div>
                        @else
                        <div style="font-weight: bold;" class="text-success">{{number_format($data->nominal, 2, '.', '')}} {{$setting->unit_point_name}}</div>
                        @endif
                     </td>
                  </tr>
                  @endforeach
