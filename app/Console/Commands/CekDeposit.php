...
class CekDeposit extends Command
{
     ...
     * @var string
     */
    protected $description = 'FOR CHECKING DEPOSIT CEKMUTASI';
                ...
                if( $mutasibank->success === true && count($mutasibank->response) > 0 )
                {
                    ...
                    if( $amount == $data_deposit->nominal_trf )
                    {
                        ...
                        try
                        {
                            ...
                            DB::commit();
                            $msg = 'deposit akun '.$setting->nama_sistem.' sebesar Rp '.number_format($amount, 0, '.', '.').' SUKSES via '.$banks->nama_bank.'. Saldo akhir: Rp '.number_format($saldo, 0, '.', '.');
                            
                            $chat = 'Yth. '.$user->name.', Deposit akun '.$setting->nama_sistem.' sebesar Rp '.number_format($amount, 0, '.', '.').' ';
                            $chat .= ' SUKSES via '.$banks->nama_bank.' ';
                            $chat .= ' Saldo akhir: Rp '.number_format($saldo, 0, '.', '.').' .';
                           
                            SMSGateway::send($user->phone, 'Yth. '.$user->name. $msg);
