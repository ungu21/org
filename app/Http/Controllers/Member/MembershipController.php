...
class MembershipController extends Controller
{
  ...
  public function index()
  {
      ...
      $min_deposit = SettingMinDeposit::all();

      return view('member.membership.index', compact('selisihhari','endedTimestamp','timestampMinus','awal','minusDay','toDay','countTransaksiSuccesBulan','countTransaksiGagalBulan','countTransaksiProsesBulan','min_deposit'));
  }
  ...
  public function upgradelevel(Request $request)
  {
    ...
    {
        //mundur 3 hari dari waktu sekarang
        $timestampMinus = date('Y-m-d', strtotime('-3 day', strtotime(date('Y-m-d'))));
    
        $count = Mutasi::where('user_id',Auth::user()->id)
                        ->where('created_at', '>=', $timestampMinus)
        ...
        if($count == 0)
        {
            return response()->json([
                'success' => false,
                'message' => 'Anda tidak melakuak deposit minimal Rp.250.000 dihitung sejak 3 hari kebelakang!.',
            ], 200);
        }
        ...
        if($currentBalance < 250000)
        {
            return response()->json([
                'success' => false,
                'message' => 'Minimal Saldo yang tersisa di akun anda Rp.250.000 untuk bisa menjadi level '.Role::find($request->to_level)->display_name.' !.',
            ], 200);
        }
    }
    ...
    if($request->to_level == $this->enterprise_role)
    {
        ...
        if($count == '0'){
            return response()->json([
                'success' => false,
                'message' => 'Tidak ditemukan histori deposit dengan minimal nominal Rp.500.000 dihitung sejak 3 hari kebelakang!.',
            ], 200);
        }
        ...
        if($currentBalance < 500000){
            return response()->json([
                'success' => false,
                'message' => 'Minimal Saldo yang tersisa di akun anda Rp.500.000 untuk bisa menjadi level '.Role::find($request->to_level)->display_name.' !.',
            ], 200);
        }
    }
