...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
    	...
    	$pembeliankategori = Pembeliankategori::where('status', 1)->limit(5)->get();
        ...
        $getimages = Banner::getImgSliderHome();
        return view('home', compact('pembeliankategori','operator', 'testimonials', 'faqs','produkPLN','tagihan_providers', 'captcha','getimages'));
    }
    ...
    public function produkLevel($id)
    {
        if(Auth::check() == false)
        {
            $produk = Personal::select('id','product_id','product_name','desc','price', 'status')->where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
        }
        ...
        {
            $produk = Agen::select('id','product_id','product_name','desc','price', 'status')->where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
        }
        elseif(Auth::user()->roles()->first()->id == $this->enterprise_role)
        {
            $produk = Enterprise::select('id', 'product_id','product_name', 'desc', 'price', 'status')->where('pembelianoperator_id', $id)->orderBy('price_default', 'ASC')->get();
        }
        else
        {
    ...
    {
        $kategoris = Pembeliankategori::where('slug', $title)->firstOrFail();
        return view('price-pembelian', compact('kategoris'));
    }

    public function pricePembayaran($title)
    {
        $kategoris = Pembayarankategori::where('slug', $title)->firstOrFail();
